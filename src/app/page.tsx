...
export default function Home() {
  ...
  useEffect(() => {
    ...
    socketInstance.on('connect', () => {
      ...
      if (gameIdRef.current && roleRef.current) {
        console.log(`Auto-joining game room: ${gameIdRef.current} as ${roleRef.current}`)
        socketInstance.emit('join-game', { gameId: gameIdRef.current, role: roleRef.current })
      }
    })
    ...
    socketInstance.on('spin-result', (data: { winner: { id: string; name: string } }) => {
    ...
    socketInstance.on('game-state', (state: { participants: Participant[]; status: string; winner: { id: string; name: string } | null }) => {
  ...
  const joinGameRoom = useCallback((gId: string, r: 'organizer' | 'participant') => {
    ...
    try {
      ...
      if (data.game) {
        ...
        if (data.game.status === 'finished' && data.game.participants) {
          const gameWinner = data.game.participants.find((p: Participant) => p.isWinner)
          if (gameWinner) {
  ...
  const handleCreateGame = async () => {
    if (pin.length < 4) {
      toast({ title: 'Error', description: 'El PIN debe tener al menos 4 caracteres', variant: 'destructive' })
      return
    }
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Error al crear el juego', variant: 'destructive' })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleStartSpin = async () => {
    if (participants.length === 0) {
      toast({ title: 'Error', description: 'No hay participantes inscritos', variant: 'destructive' })
      return
    }
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Error al iniciar el sorteo', variant: 'destructive' })
    } finally {
      setIsLoading(false)
    }
  }

  const handleSpinComplete = useCallback(async (spinWinner: { id: string; name: string }) => {
    ...
    if (socketRef.current && socketRef.current.connected) {
      socketRef.current.emit('winner-selected', { gameId, winner: spinWinner })
    }
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (!data.participants || data.participants.length === 0) {
        toast({ title: 'Error', description: 'No hay participantes para exportar', variant: 'destructive' })
        return
      }
      ...
      const colWidths = Object.keys(data.participants[0] || {}).map(key => ({
        wch: Math.max(key.length, ...data.participants.map((p: Record<string, unknown>) => String(p[key] || '').length))
      }))
      ws['!cols'] = colWidths

      XLSX.writeFile(wb, `sorteo_participantes_${new Date().toISOString().slice(0, 10)}.xlsx`)
      toast({ title: 'Éxito', description: 'Excel descargado correctamente' })
    } catch (error) {
      console.error('Error exporting:', error)
      toast({ title: 'Error', description: 'Error al descargar Excel', variant: 'destructive' })
    }
  }
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      // Update URL
      window.history.pushState({}, '', `?game=${data.game.id}&admin=${data.game.pin}`)
      ...
      toast({ title: 'Error', description: 'Error al reiniciar el sorteo', variant: 'destructive' })
    } finally {
      setIsLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#0a0a1a] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-md w-full">
          <button onClick={() => setView('welcome')} className="text-[#8888aa] hover:text-[#00ff8a] mb-6 flex items-center gap-2 transition-colors">
            ← Volver
          </button>

          <h2 className="text-3xl font-bold mb-2 gradient-text">Crear Sorteo</h2>
          <p className="text-[#8888aa] mb-8 text-sm">Define un PIN para acceder como organizador</p>

          <div className="space-y-6">
            <div className="neon-border rounded-xl p-6 bg-[#111127]">
              <Label htmlFor="pin" className="text-[#00ff8a] text-sm font-medium mb-2 block">PIN de Organizador</Label>
              <Input
                id="pin"
                type="text"
                placeholder="Ej: 1234"
                ...
                maxLength={8}
              />
              <p className="text-[#8888aa] text-xs mt-2">Mínimo 4 caracteres. Este PIN te dará acceso exclusivo como organizador.</p>
            </div>
      ...
      <div className="min-h-screen flex flex-col p-4 md:p-6 bg-[#0a0a1a] relative overflow-hidden">
        ...
        <div className="relative z-10 flex-1 flex flex-col">
          ...
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-xl md:text-2xl font-bold gradient-text">SORTEO</h1>
            <div className="flex items-center gap-2">
              <span className="text-[#8888aa] text-sm">PIN:</span>
              <span className="text-[#00ff8a] font-mono font-bold text-sm neon-text px-2 py-1 bg-[#111127] rounded">{game?.pin}</span>
            </div>
          </div>
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div className="flex flex-col items-center gap-4">
              <div className={`rounded-2xl p-6 bg-[#111127] w-full flex flex-col items-center ${spinning ? 'neon-border-purple' : 'neon-border animate-pulse-neon'}`}>
                <p className="text-[#00ff8a] font-bold text-sm mb-4 uppercase tracking-wider">Código QR para Participar</p>
              ...
              <div className="gradient-border rounded-xl p-4 bg-[#111127] w-full text-center">
                <p className="text-[#8888aa] text-xs uppercase tracking-wider mb-1">Participantes Inscritos</p>
                <p className="text-4xl font-bold text-[#00ff8a] neon-text">{participantCount}</p>
              </div>

              {/* Participant List */}
              <div className="neon-border rounded-xl p-4 bg-[#111127] w-full max-h-48 overflow-y-auto">
                <p className="text-[#8888aa] text-xs uppercase tracking-wider mb-2">Lista de Participantes</p>
                {participants.length === 0 ? (
                  <p className="text-[#555577] text-sm text-center py-2">Esperando participantes...</p>
                ) : (
                  <div className="space-y-1">
                    {participants.map((p, i) => (
                      <div key={p.id} className="flex items-center gap-2 text-sm py-1 border-b border-[#1a1a3e] last:border-0">
                        <span className="text-[#8e00ff] font-mono text-xs">{i + 1}.</span>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#0a0a1a] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-md w-full">
          ...
          <h1 className="text-3xl font-bold mb-2 gradient-text text-center">SORTEO</h1>
          <p className="text-[#8888aa] mb-6 text-center text-sm">Completa tus datos para participar</p>

          <div className="neon-border rounded-2xl p-6 bg-[#111127] space-y-4">
            <div>
              <Label htmlFor="name" className="text-[#00ff8a] text-sm font-medium mb-1.5 block">Nombre</Label>
              <Input
                id="name"
                type="text"
            ...
            <div>
              <Label htmlFor="email" className="text-[#00ff8a] text-sm font-medium mb-1.5 block">Correo Electrónico</Label>
              <Input
                id="email"
                type="email"
            ...
            <div>
              <Label htmlFor="whatsapp" className="text-[#00ff8a] text-sm font-medium mb-1.5 block">WhatsApp</Label>
              <Input
                id="whatsapp"
                type="tel"
            ...
            <Button
              ...
              disabled={isLoading || !formName.trim() || !formEmail.trim() || !formWhatsapp.trim()}
              className="w-full py-6 text-lg font-bold bg-gradient-to-r from-[#8e00ff] to-[#00ff8a] hover:opacity-90 text-[#0a0a1a] rounded-xl transition-all duration-300 disabled:opacity-50"
            >
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#0a0a1a] relative overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-md w-full">
          <div className="neon-border rounded-2xl p-8 bg-[#111127] animate-pulse-neon">
            ...
            <h2 className="text-2xl font-bold text-[#00ff8a] neon-text mb-2">¡Estás Inscrito!</h2>
            <p className="text-[#8888aa] text-sm mb-6">
              Espera a que el organizador inicie el sorteo. Verás el resultado aquí en vivo.
            </p>
            <div className="flex items-center justify-center gap-2">
              <div className="w-2 h-2 bg-[#00ff8a] rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
              <div className="w-2 h-2 bg-[#00ff8a] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
              <div className="w-2 h-2 bg-[#00ff8a] rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
            </div>
          </div>
        </div>
      </div>
